...
public class EmploymentStatusChartQueryHandler(
    ...
    : IQueryHandler<EmploymentStatusChartQuery, IResult<EmploymentStatusChartQueryResult>>
{
    public async Task<IResult<EmploymentStatusChartQueryResult>> Handle(EmploymentStatusChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetEmploymentStatusChart(filter, cancellationToken);
        
        var result = new EmploymentStatusChartQueryResult(chartData.IsSeekingForJob, chartData.IsHiring);
        
        return SuccessResult<EmploymentStatusChartQueryResult>.Create(result);
    }
}