...
namespace Networkie.Application.Features.Dashboard.Charts.Queries.DepartmentChart;

public class DepartmentChartQueryHandler(
    ...
    : IQueryHandler<DepartmentChartQuery, IResult<IEnumerable<DepartmentChartQueryResult>>>
{
    public async Task<IResult<IEnumerable<DepartmentChartQueryResult>>> Handle(DepartmentChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetDepartmentChart(filter, cancellationToken);
        
        var result = chartData.Select(x => new DepartmentChartQueryResult(x.DepartmentName, x.Count));
        
        return SuccessResult<IEnumerable<DepartmentChartQueryResult>>.Create(result);
    }
}