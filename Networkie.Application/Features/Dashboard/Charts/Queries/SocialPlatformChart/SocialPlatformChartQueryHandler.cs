...
public class SocialPlatformChartQueryHandler(
    ...
    : IQueryHandler<SocialPlatformChartQuery, IResult<IEnumerable<SocialPlatformChartQueryResult>>>
{
    public async Task<IResult<IEnumerable<SocialPlatformChartQueryResult>>> Handle(SocialPlatformChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetSocialPlatformChart(filter, cancellationToken);
        
        var result = chartData.Select(x => new SocialPlatformChartQueryResult(x.SocialPlatformName, x.Count));
        
        return SuccessResult<IEnumerable<SocialPlatformChartQueryResult>>.Create(result);
    }
}