...
public class UniversityChartQueryHandler(
    ...
    : IQueryHandler<UniversityChartQuery, IResult<IEnumerable<UniversityChartQueryResult>>>
{
    public async Task<IResult<IEnumerable<UniversityChartQueryResult>>> Handle(UniversityChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetUniversityChart(filter, cancellationToken);
        
        var result = chartData.Select(x => new UniversityChartQueryResult(x.UniversityName, x.Count));
        
        return SuccessResult<IEnumerable<UniversityChartQueryResult>>.Create(result);
    }
}