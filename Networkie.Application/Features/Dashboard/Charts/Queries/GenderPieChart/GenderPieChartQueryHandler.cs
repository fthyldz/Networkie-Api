...
namespace Networkie.Application.Features.Dashboard.Charts.Queries.GenderPieChart;

public class GenderPieChartQueryHandler(
    ...
    : IQueryHandler<GenderPieChartQuery, IResult<GenderPieChartQueryResult>>
{
    public async Task<IResult<GenderPieChartQueryResult>> Handle(GenderPieChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetGenderPieChart(filter, cancellationToken);
        
        var result = new GenderPieChartQueryResult(chartData.MaleCount, chartData.FemaleCount);
        
        return SuccessResult<GenderPieChartQueryResult>.Create(result);
    }
}