...
public class AgeBarChartQueryHandler(
    ...
    : IQueryHandler<AgeBarChartQuery, IResult<IEnumerable<AgeBarChartQueryResult>>>
{
    public async Task<IResult<IEnumerable<AgeBarChartQueryResult>>> Handle(AgeBarChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetAgeBarChart(filter, cancellationToken);
        
        var result = chartData.Select(x => new AgeBarChartQueryResult(x.Age, x.Count));
        
        return SuccessResult<IEnumerable<AgeBarChartQueryResult>>.Create(result);
    }
}