...
public class CityChartQueryHandler(
    ...
    : IQueryHandler<CityChartQuery, IResult<IEnumerable<CityChartQueryResult>>>
{
    public async Task<IResult<IEnumerable<CityChartQueryResult>>> Handle(CityChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetCityChart(filter, cancellationToken);
        
        var result = chartData.Select(x => new CityChartQueryResult(x.CityName, x.Count));
        
        return SuccessResult<IEnumerable<CityChartQueryResult>>.Create(result);
    }
}