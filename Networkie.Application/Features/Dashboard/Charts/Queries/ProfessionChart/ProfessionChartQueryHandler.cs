...
public class ProfessionChartQueryHandler(
    ...
    : IQueryHandler<ProfessionChartQuery, IResult<IEnumerable<ProfessionChartQueryResult>>>
{
    public async Task<IResult<IEnumerable<ProfessionChartQueryResult>>> Handle(ProfessionChartQuery query, CancellationToken cancellationToken)
    {
        ...
        var chartData = await userRepository.GetProfessionChart(filter, cancellationToken);
        
        var result = chartData.Select(x => new ProfessionChartQueryResult(x.ProfessionName, x.Count));
        
        return SuccessResult<IEnumerable<ProfessionChartQueryResult>>.Create(result);
    }
}